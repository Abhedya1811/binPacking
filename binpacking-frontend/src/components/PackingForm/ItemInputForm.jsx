...
const ItemInputForm = ({ items = [], onItemsChange, onImport, onTemplate }) => {
  ...
  const toDisplayUnit = (valueInMeters) => {
    return Number((valueInMeters / UNIT_CONVERSIONS[unit]).toFixed(3));
  };

  // Convert from display unit to meters
  const toMeters = (valueInDisplayUnit) => {
    return valueInDisplayUnit * UNIT_CONVERSIONS[unit];
  };

  // Convert from import unit to meters
  const fromImportUnitToMeters = (valueInImportUnit) => {
    return valueInImportUnit * UNIT_CONVERSIONS[importUnit];
  };

  // Convert template items to display unit for preview
  const getTemplateItemsInDisplayUnit = (templateType) => {
    return itemTemplates[templateType].map(item => ({
      ...item,
      width: toDisplayUnit(item.width),
      height: toDisplayUnit(item.height),
      depth: toDisplayUnit(item.depth)
    }));
  };
  ...
  const processImport = (rows) => {
    ...
    rows.forEach((row, index) => {
      ...
      // Read dimensions from file (assume they're in the selected import unit)
      const widthInImportUnit = round3(row['WIDTH(m)'] || row['Width'] || row.width || row['WIDTH'] || 0.5);
      const depthInImportUnit = round3(row['DEPTH(m)'] || row['Depth'] || row.depth || row['DEPTH'] || 0.5);
      const heightInImportUnit = round3(row['HEIGHT(m)'] || row['Height'] || row.height || row['HEIGHT'] || 0.5);
      const quantity = parseInt(row['QUANTITY'] || row['Quantity'] || row.quantity || 1);

      // Convert from import unit to meters for internal storage
      const widthInMeters = fromImportUnitToMeters(widthInImportUnit);
      const depthInMeters = fromImportUnitToMeters(depthInImportUnit);
      const heightInMeters = fromImportUnitToMeters(heightInImportUnit);

      console.log('Importing item:', {
        name: originalName,
        importUnit: UNIT_DISPLAY[importUnit],
        widthInImportUnit,
        depthInImportUnit,
        heightInImportUnit,
        widthInMeters,
      ...
      // Validate dimensions (check the import unit values, not meters)
      if (widthInImportUnit <= 0) {
        issues.push({
          type: "invalid_dimensions",
          itemName: originalName,
          message: `Width must be greater than 0 ${UNIT_DISPLAY[importUnit]}`
        });
      }
      
      if (depthInImportUnit <= 0) {
        issues.push({
          type: "invalid_dimensions",
          itemName: originalName,
          message: `Depth must be greater than 0 ${UNIT_DISPLAY[importUnit]}`
        });
      }
      
      if (heightInImportUnit <= 0) {
        issues.push({
          type: "invalid_dimensions",
          itemName: originalName,
          message: `Height must be greater than 0 ${UNIT_DISPLAY[importUnit]}`
        });
      }
      ...
      // Check for duplicates using the meter values for accuracy
      const duplicate = items.find(
        (item) =>
          item.name === cleanedName &&
          Math.abs(item.width - widthInMeters) < 0.001 &&
          Math.abs(item.height - heightInMeters) < 0.001 &&
          Math.abs(item.depth - depthInMeters) < 0.001
      );
      ...
      if (cleanedName) {
        processed.push({
          ...
          name: cleanedName,
          width: widthInMeters > 0 ? widthInMeters : 0.5,
          height: heightInMeters > 0 ? heightInMeters : 0.5,
          depth: depthInMeters > 0 ? depthInMeters : 0.5,
          quantity: quantity > 0 ? quantity : 1,
          weight: 0,
          fragile: false,
          color: generateRandomColor(),
          // Store original import values for display in preview
          _importWidth: widthInImportUnit,
          _importDepth: depthInImportUnit,
          _importHeight: heightInImportUnit
        });
      }
    });
  ...
  const handleImportWithUnit = () => {
    if (importSummary.rawData) {
      ...
      setImportSummary(prev => ({ ...prev, rawData: undefined }));
    }
  };
  ...
  const handleUpdateImportedItem = (index, field, value) => {
    ...
    if (field === 'width' || field === 'height' || field === 'depth') {
      ...
      // When updating in preview, update both the meter value and store the import unit value
      updatedProcessed[index][field] = value; // This is in meters
      // Update the import display value
      updatedProcessed[index][`_import${field.charAt(0).toUpperCase() + field.slice(1)}`] = 
        toDisplayUnit(value);
    ...
    const duplicate = items.find(
      (item) =>
        ...
        Math.abs(item.width - updatedProcessed[index].width) < 0.001 &&
        Math.abs(item.height - updatedProcessed[index].height) < 0.001 &&
        Math.abs(item.depth - updatedProcessed[index].depth) < 0.001
    );
  ...
  return (
    <Box className="item-input-container">
      {/* Header with Stats and Unit Selector */}
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="subtitle1" fontWeight="600" gutterBottom>
            ...
            Add New Item ({UNIT_DISPLAY[unit]})
          </Typography>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={4}>
              <TextField
                fullWidth
                size="small"
                label={`Width (${UNIT_DISPLAY[unit]})`}
                type="number"
                value={toDisplayUnit(newItem.width)}
                onChange={(e) =>
                  setNewItem({ 
                    ...newItem, 
                    width: toMeters(parseFloat(e.target.value) || 0)
                  })
                }
                InputProps={{
                  endAdornment: <InputAdornment position="end">{UNIT_DISPLAY[unit]}</InputAdornment>
                }}
                inputProps={{ step: unit === 'meters' ? 0.01 : unit === 'cm' ? 1 : 0.1, min: 0.01 }}
              />
            </Grid>
            ...
            <Grid item xs={6} sm={4}>
              <TextField
                fullWidth
                size="small"
                label={`Height (${UNIT_DISPLAY[unit]})`}
                type="number"
                value={toDisplayUnit(newItem.height)}
                onChange={(e) =>
                  setNewItem({ 
                    ...newItem, 
                    height: toMeters(parseFloat(e.target.value) || 0)
                  })
                }
                InputProps={{
                  endAdornment: <InputAdornment position="end">{UNIT_DISPLAY[unit]}</InputAdornment>
            ...
            <Grid item xs={6} sm={5}>
              <TextField
                fullWidth
                size="small"
                label={`Depth (${UNIT_DISPLAY[unit]})`}
                type="number"
                value={toDisplayUnit(newItem.depth)}
                onChange={(e) =>
                  setNewItem({ 
                    ...newItem, 
                    depth: toMeters(parseFloat(e.target.value) || 0)
                  })
                }
                InputProps={{
                  endAdornment: <InputAdornment position="end">{UNIT_DISPLAY[unit]}</InputAdornment>
              ...
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="center">Width ({UNIT_DISPLAY[unit]})</TableCell>
                    <TableCell align="center">Height ({UNIT_DISPLAY[unit]})</TableCell>
                    <TableCell align="center">Depth ({UNIT_DISPLAY[unit]})</TableCell>
                  ...
                  {items.map((item, index) => (
                    <TableRow key={item.id || index} hover>
                      ...
                      <TableCell align="center">
                        <TextField
                          type="number"
                          value={toDisplayUnit(item.width)}
                          onChange={(e) => handleUpdateItem(index, 'width', toMeters(parseFloat(e.target.value) || 0))}
                          size="small"
                          sx={{ width: 80 }}
                          inputProps={{ step: unit === 'meters' ? 0.01 : unit === 'cm' ? 1 : 0.1 }}
                          variant="standard"
                          InputProps={{ 
                      ...
                      <TableCell align="center">
                        <TextField
                          type="number"
                          value={toDisplayUnit(item.height)}
                          onChange={(e) => handleUpdateItem(index, 'height', toMeters(parseFloat(e.target.value) || 0))}
                          size="small"
                          sx={{ width: 80 }}
                          inputProps={{ step: unit === 'meters' ? 0.01 : unit === 'cm' ? 1 : 0.1 }}
                          variant="standard"
                      ...
                      <TableCell align="center">
                        <TextField
                          type="number"
                          value={toDisplayUnit(item.depth)}
                          onChange={(e) => handleUpdateItem(index, 'depth', toMeters(parseFloat(e.target.value) || 0))}
                          size="small"
                          sx={{ width: 80 }}
                          inputProps={{ step: unit === 'meters' ? 0.01 : unit === 'cm' ? 1 : 0.1 }}
                          variant="standard"
      ...
      {/* Import Summary Dialog with Unit Selector */}
      <Dialog 
      ...
      >
        <DialogTitle sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            ...
            {/* Import Unit Selector - Show if raw data exists (before processing) */}
            {importSummary.rawData && (
              <Box display="flex" alignItems="center" gap={1}>
                <Typography variant="body2" color="text.secondary">
                  Dimensions in file are in:
                </Typography>
                {Object.keys(UNIT_CONVERSIONS).map(unitType => (
                  <Chip
                    key={unitType}
                    label={UNIT_DISPLAY[unitType]}
                    onClick={() => setImportUnit(unitType)}
                    variant={importUnit === unitType ? 'filled' : 'outlined'}
                    color={importUnit === unitType ? 'primary' : 'default'}
                    size="small"
                    sx={{ minWidth: 45 }}
                  />
                ))}
        ...
        <DialogContent dividers sx={{ p: 0 }}>
          {/* Show unit selector prompt if raw data exists */}
          {importSummary.rawData && (
            <Box sx={{ p: 4, textAlign: 'center' }}>
              <Typography variant="body1" gutterBottom>
                Please select the unit of measurement used in your file
              </Typography>
              <Typography variant="caption" color="text.secondary">
                The dimensions will be converted to meters for internal storage
              </Typography>
            </Box>
          )}

          {/* Issues Summary - Only show after processing */}
          {!importSummary.rawData && importSummary.issues.length > 0 && (
          ...
          {/* Items Table - Show after processing */}
          {!importSummary.rawData && importSummary.processedItems.length > 0 ? (
            <>
              <Box sx={{ p: 2, bgcolor: '#f5f5f5', borderBottom: 1, borderColor: 'divider' }}>
                ...
                <Typography variant="caption" color="text.secondary">
                  Values shown in meters (internal storage). Edit if needed.
                </Typography>
              </Box>
